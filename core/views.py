...
def _background_pipeline(job_id):
    ...
    try:
        ...
        print(f"☁️ Cloudinary Video URL: {video_url}")

        if not video_url:
            raise ValueError("Cloudinary video URL is empty.")

        # التأكد أن الرابط رابط HTTP/HTTPS كامل
        if not video_url.startswith(("http://", "https://")):
            raise ValueError(f"Invalid Cloudinary video URL: {video_url}")

        video_path = os.path.join(temp_dir, "input_video.mp4")

        print(f"⬇️ Downloading video from Cloudinary: {video_url}")

        download_file_from_url(video_url, video_path)

        print(f"✅ Video downloaded to temp: {video_path}")
        ...
        print(f"🎬 Final video created locally: {out_path}")

        # 🎉 النجاح الكامل وحفظ المسار الناتج
        # رفع الفيديو النهائي إلى Cloudinary
        with open(out_path, "rb") as video_file:
            job.output_video.save(out_filename, video_file, save=False)

        print(f"☁️ Final dubbed video uploaded to Cloudinary: {job.output_video.url}")
        ...
        if job.video_file:
            print(f"🗑️ Deleting original video from Cloudinary: {job.video_file.name}")

            job.video_file.delete(save=False)

            print("✅ Original video deleted from Cloudinary.")
        ...
        if out_path and os.path.exists(out_path):
            ...
            print(f"🗑️ Local final video deleted: {out_path}")
        ...
        if out_path and os.path.exists(out_path):
            try:
                ...
                print(f"🧹 Final temporary output deleted: {out_path}")

            except Exception as cleanup_error:
                print(f"⚠️ فشل حذف ملف الفيديو النهائي المؤقت: {cleanup_error}")
...
def upload_video_api(request):
    ...
    job = DubbingJob.objects.create(
        video_file=video_file, status="PENDING", status_text="Waiting in queue..."
    )

    # threading.Thread(target=_background_pipeline, args=(str(job.id),)).start()
    worker = threading.Thread(
        target=_background_pipeline, args=(str(job.id),), daemon=True
    )
...
def download_video_api(request, job_id):
    ...
    if not job.output_video:
        return JsonResponse({"error": "Output video not found"}, status=404)

    return JsonResponse({"output_video_url": job.output_video.url})
...
#         with open(out_path, "rb") as video_file:
#             job.output_video.save(out_filename, video_file)
