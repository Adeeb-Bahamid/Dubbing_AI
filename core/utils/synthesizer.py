...
def generate_arabic_audio_track(translated_segments, temp_dir):
    ...
    for idx, seg in enumerate(translated_segments):
        ...
        output_path = os.path.join(temp_dir, f"sub_{idx}.mp3")

        semaphore = asyncio.Semaphore(10)
        tasks.append(_tts_worker(text, output_path, semaphore))

    try:
        asyncio.run(_generate_all_tasks(tasks))

        print("✅ All TTS files generated successfully.")

    except Exception as e:  # noqa: BLE001
        print(f"❌ Parallel TTS Error: {e}", file=sys.stderr)
